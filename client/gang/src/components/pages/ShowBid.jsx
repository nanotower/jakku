...
export default class ShowBid extends Component {
 ...
 extractCoordinates(latlng) {
   console.log("extract coordinate", latlng)
  const lat= latlng[0];
  const lng= latlng[1];
  return [{lat, lng}];
}
  ...
  render() {
    return (
      <React.Fragment>
        ...
        <Bidmapcontainer position={[{lat: this.props.user.bid.location.coordinates[0], lng: this.props.user.bid.location.coordinates[1]}]}></Bidmapcontainer>
